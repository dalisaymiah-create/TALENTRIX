...
$auth->requireAdminType('sports');

// Get statistics
$stats = [];

// Total sports students
$stmt = $pdo->query("
    ...
    WHERE EXISTS (
        ...
        WHERE p.student_id = s.id AND a.activity_type_filter = 'sports'
    ) OR s.course LIKE '%Sports%' OR s.course LIKE '%Physical%'
");
$stats['total_students'] = $stmt->fetch()['total'];

// Total sports coaches
$stmt = $pdo->query("
    SELECT COUNT(*) as total FROM Coach c
    WHERE c.specialization ILIKE '%sports%' OR c.specialization ILIKE '%basketball%' 
       OR c.specialization ILIKE '%volleyball%' OR c.specialization ILIKE '%swimming%'
");
$stats['total_coaches'] = $stmt->fetch()['total'];

// Total sports participations
$stmt = $pdo->query("
    SELECT COUNT(*) as total FROM Participation p
    JOIN Contest c ON p.contest_id = c.id
    JOIN Activity a ON c.activity_id = a.id
    WHERE a.activity_type_filter = 'sports'
");
...
$stmt = $pdo->query("
    ...
    AND (
        (u.usertype = 'student' AND (
            s.course LIKE '%Sports%' OR s.course LIKE '%Physical%' OR
            EXISTS (SELECT 1 FROM Participation p JOIN Contest ct ON p.contest_id = ct.id JOIN Activity a ON ct.activity_id = a.id WHERE p.student_id = s.id AND a.activity_type_filter = 'sports')
        ))
        OR
        (u.usertype = 'coach' AND (c.specialization ILIKE '%sports%' OR c.specialization ILIKE '%basketball%' OR c.specialization ILIKE '%volleyball%'))
    )
    ORDER BY u.id DESC
");
$users = $stmt->fetchAll();
...
$stmt = $pdo->query("SELECT * FROM Category WHERE category_type IN ('sports', 'general') ORDER BY category_name");
$categories = $stmt->fetchAll();

// Get recent sports participations
$stmt = $pdo->query("
...
// Get announcements for sports
$stmt = $pdo->query("
    SELECT a.*, u.email as author_email
    FROM Announcement a
    LEFT JOIN \"User\" u ON a.author_id = u.id
    WHERE a.announcement_type IN ('sports', 'general')
    ...
    if (isset($_POST['action'])) {
        try {
            switch ($_POST['action']) {
                ...
                case 'add_category':
                    $stmt = $pdo->prepare("
                        INSERT INTO Category (category_name, category_type) VALUES (?, 'sports')
                    ");
...
case 'delete_user':
    ...
    break;
                ...
                case 'add_announcement':
                    ...
                    $announcementType = isset($_POST['announcement_type']) ? $_POST['announcement_type'] : 'sports';
...
<head>
    ...
    <title>Sports Admin Dashboard - BISU Candijay</title>
...
<div class="dashboard-wrapper">
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-nav">
            ...
            <a onclick="showSection('users')"><i class="fas fa-users"></i> Athletes & Coaches</a>
            <a onclick="showSection('activities')"><i class="fas fa-calendar-alt"></i> Sports Activities</a>
            <a onclick="showSection('contests')"><i class="fas fa-trophy"></i> Sports Contests</a>
            <a onclick="showSection('categories')"><i class="fas fa-tags"></i> Sports Categories</a>
    ...
    <div class="main-content">
        <div class="top-bar">
            <div class="page-title">
                <h2><i class="fas fa-futbol"></i> Sports Admin Dashboard</h2>
                <p>Managing Sports and Athletic Achievements</p>
            </div>
            <div class="user-info">
        ...
        <div id="overview-section" class="section">
            ...
            <div class="data-table">
                ...
                <?php $recentAnnouncements = array_slice($announcements, 0, 2); ?>
                        ...
                        <div class="announcement-preview">
                                    ...
                                    <?php if (count($announcement['images']) > 1): ?>
                                        <span style="position: relative; display: inline-block;">
                                            ...
                                            <?php if (count($announcement['images']) > 2): ?>
                                                <span style="position: absolute; bottom: 3px; right: 3px; background: rgba(0,0,0,0.7); color: white; padding: 1px 4px; border-radius: 8px; font-size: 8px;">+<?php echo count($announcement['images']) - 2; ?></span>
                                            <?php endif; ?>
                                        </span>
                                    <?php endif; ?>
                                </div>
                            ...
                            <div class="announcement-preview-content">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; flex-wrap: wrap; gap: 6px;">
                                    <h4 style="color: #333; margin: 0; font-size: 0.85rem;"><?php echo htmlspecialchars($announcement['title']); ?></h4>
                                    <span class="badge <?php echo $announcement['is_published'] ? 'badge-published' : 'badge-draft'; ?>" style="font-size: 0.65rem;">
                                        <?php echo $announcement['is_published'] ? 'Published' : 'Draft'; ?>
                                    </span>
                                </div>
                                <p style="color: #666; margin-bottom: 6px; font-size: 0.7rem;"><?php echo htmlspecialchars(substr($announcement['content'], 0, 80)) . '...'; ?></p>
                                <small style="color: #999; font-size: 0.65rem;">Posted: <?php echo date('M d, Y', strtotime($announcement['created_at'])); ?></small>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p style="font-size: 0.8rem;">No announcements yet. Click "Announcements" to create one.</p>
                <?php endif; ?>
            </div>
            
            <div class="data-table">
                <h3><i class="fas fa-clock"></i> Recent Sports Participations</h3>
                <table style="font-size: 0.75rem;">
                    ...
                    <tbody>
                        <?php foreach (array_slice($recentParticipations, 0, 4) as $participation): ?>
                        <tr>
                        ...
                        <tr>
                            <td colspan="6" style="text-align:center;">No sports participations recorded yet.</td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
        ...
        <div id="announcements-section" class="section" style="display: none;">
            <div class="form-card">
                ...
                <form method="POST" enctype="multipart/form-data" id="announcementForm">
                    ...
                    <input type="hidden" name="announcement_type" value="sports">
                    ...
                    <div class="form-group">
                        <label>Content *</label>
                        <textarea name="content" rows="3" placeholder="Write your announcement here..." required style="width: 100%; padding: 8px; border: 2px solid #e0e0e0; border-radius: 10px;"></textarea>
                    </div>
                    <div class="form-group">
                        ...
                        <small style="color: #6c757d; font-size: 0.7rem;">Accepted formats: JPG, PNG, GIF, WEBP. Max size per image: 5MB. You can select multiple images.</small>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if (!empty($announcement['images'])): ?>
                                        <div class="announcement-images-gallery">
                                            <?php foreach (array_slice($announcement['images'], 0, 2) as $idx => $img): ?>
                                                <img src="../<?php echo $img; ?>" class="gallery-image" style="width: 40px; height: 40px;" alt="Image" onclick="viewFullImage('<?php echo $img; ?>')">
                                            <?php endforeach; ?>
                                            <?php if (count($announcement['images']) > 2): ?>
                                                <span style="display: inline-flex; align-items: center; justify-content: center; width: 40px; height: 40px; background: #e9ecef; border-radius: 6px; font-size: 0.7rem;">+<?php echo count($announcement['images']) - 2; ?></span>
                                            <?php endif; ?>
                                        </div>
                                    <?php else: ?>
                                ...
                                <td>
                                    <form method="POST" style="display: inline;">
                                        ...
                                        <button type="submit" class="<?php echo $announcement['is_published'] ? 'btn-warning' : 'btn-success'; ?>" style="padding: 3px 8px; font-size: 0.7rem;">
                                ...
                                <td>
                                    <form method="POST" style="display: inline;" onsubmit="return confirm('Delete this announcement and all its images?')">
                                        ...
                                        <input type="hidden" name="announcement_id" value="<?php echo $announcement['id']; ?>">
                                        <button type="submit" class="btn-danger" style="padding: 3px 8px; font-size: 0.7rem;">Delete</button>
                                    </form>
                                </td>
                            </tr>
                            <?php endforeach; ?>
        ...
        <div id="users-section" class="section" style="display: none;">
            <div class="data-table">
                <h3><i class="fas fa-users"></i> Athletes & Coaches (Sports Division)</h3>
        ...
        <div id="activities-section" class="section" style="display: none;">
            <div class="form-card">
                <h3><i class="fas fa-plus-circle"></i> Add New Sports Activity</h3>
                <form method="POST">
                    <input type="hidden" name="action" value="add_activity">
                    <div class="form-row">
                        <div class="form-group">
                            <label>Activity Name</label>
                            <input type="text" name="activity_name" placeholder="e.g., Palarong BISU, Intramurals" required>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="event_name" placeholder="e.g., Basketball Tournament">
                        </div>
                        <div class="form-group">
                            <label>Year</label>
            ...
            <div class="data-table">
                <h3><i class="fas fa-calendar-alt"></i> Existing Sports Activities</h3>
                    ...
                    <table style="font-size: 0.75rem;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span class="badge badge-sports"><?php echo ucfirst($activity['activity_type']); ?></span></td>
        ...
        <div id="contests-section" class="section" style="display: none;">
            <div class="form-card">
                <h3><i class="fas fa-plus-circle"></i> Add New Sports Contest</h3>
                <form method="POST">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" placeholder="e.g., Men's Basketball" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
            ...
            <div class="data-table">
                <h3><i class="fas fa-trophy"></i> Existing Sports Contests</h3>
                <div style="max-height: 350px; overflow-y: auto;">
                    <table style="font-size: 0.75rem;">
                        <thead>
                            <tr>
        ...
        <div id="categories-section" class="section" style="display: none;">
            <div class="form-card">
                <h3><i class="fas fa-plus-circle"></i> Add New Sports Category</h3>
                <form method="POST">
                    <input type="hidden" name="action" value="add_category">
                    <div class="form-group">
                        <label>Category Name</label>
                        <input type="text" name="category_name" placeholder="e.g., Basketball, Volleyball, Swimming" required>
                    </div>
            ...
            <div class="data-table">
                <h3><i class="fas fa-tags"></i> Existing Sports Categories</h3>
                <div style="max-height: 400px; overflow-y: auto;">
                    <table style="font-size: 0.75rem;">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <?php foreach ($categories as $category): ?>
                            <tr>
                                <td><span class="badge badge-sports"><?php echo htmlspecialchars($category['category_name']); ?></span></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
        ...
        <div id="participations-section" class="section" style="display: none;">
            <div class="data-table">
                <h3><i class="fas fa-list"></i> Sports Participations</h3>
        ...
        <div id="reports-section" class="section" style="display: none;">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select id="coachSelect">
                                ...
                                ");
                                $sportsCoaches = $coachStmt->fetchAll();
                                foreach ($sportsCoaches as $coach): ?>
                                    <option value="<?php echo $coach['id']; ?>">
                                        <?php echo htmlspecialchars($coach['first_name'] . ' ' . $coach['last_name'] . ' (' . ($coach['specialization'] ?: 'Sports') . ')'); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select id="eventSelect">
                                ...
                                ");
                                $sportsEvents = $eventStmt->fetchAll();
                        ...
                        <div class="form-group">
                            ...
                            <select id="teamSelect">
                                ...
                                $teamStmt = $pdo->query("
                                    ...
                                    WHERE t.specialization ILIKE '%sports%' OR t.specialization ILIKE '%basketball%' 
                                       OR t.specialization ILIKE '%volleyball%' OR t.specialization ILIKE '%swimming%'
                                    ORDER BY t.team_name
                                ");
                                $sportsTeams = $teamStmt->fetchAll();
                                foreach ($sportsTeams as $team): ?>
                                    <option value="<?php echo $team['id']; ?>">
                                        <?php echo htmlspecialchars($team['team_name'] . ' (' . ($team['specialization'] ?: 'Sports') . ')'); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
            ...
            <div class="data-table" id="reportPreview" style="display: none;">
                ...
                <div id="reportPreviewContent" style="max-height: 300px; overflow-y: auto;"></div>
                <button id="downloadReportBtn" class="btn-primary" style="margin-top: 12px;"><i class="fas fa-download"></i> Download CSV</button>
            </div>
        </div>
    </div>
</div>
...
function downloadReportCSV() {
    ...
    link.href = url;
    link.setAttribute('download', 'sports_report_' + currentReportType + '_' + new Date().toISOString().slice(0,19) + '.csv');
