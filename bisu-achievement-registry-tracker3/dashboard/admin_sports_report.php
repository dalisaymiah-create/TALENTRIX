<?php
// admin_sports_report.php - Report for Sports Admin only with filters
...
// Sports Admin specific - only Sports and Athletic
$allowed_category_types = ['Sport', 'Athletic'];
$report_title = 'Sports and Athletic Report';
...
// Get available categories (sports/events) for filter
$availableCategories = [];
try {
    $stmt = $pdo->prepare("
        SELECT DISTINCT cat.id, cat.category_name, cat.category_type
        FROM category cat
        WHERE cat.category_type IN ('Sport', 'Athletic')
        ORDER BY cat.category_type, cat.category_name
    ");
    $stmt->execute();
...
// =====================================================
$where_conditions = ["p.membership_status = 'active'", "cat.category_type IN ('Sport','Athletic')"];
$params = [];
...
try {
    $participants_sql = "
        ...
        AND cat.category_type IN ('Sport','Athletic')
    ";
...
$selected_category_name = 'All Sports/Athletic';
if ($selected_category !== 'all') {
...
<head>
    ...
    <title>Sports Admin Report - BISU Candijay Achievement Tracker</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .category-badge-sport { background: #d4edda; color: #155724; }
        .category-badge-athletic { background: #d1ecf1; color: #0c5460; }
        ...
        @media print {
            ...
            /* Keep badges visible */
            .ranking-badge, .competition-badge, .category-badge-sport, .category-badge-athletic {
    ...
    <div class="dashboard-container">
        ...
        <div class="sidebar">
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item"><a href="admin_sports_analytics.php"><i class="fas fa-chart-bar"></i> Analytics</a></li>
                ...
                <li class="nav-item active"><a href="admin_sports_report.php"><i class="fas fa-print"></i> Reports</a></li>
        ...
        <div class="main-content">
            <div class="top-bar">
                <div class="page-title">
                    <h1><i class="fas fa-print"></i> Reports | <span class="admin-badge"><i class="fas fa-futbol"></i> SPORTS ADMIN</span></h1>
                </div>
                <div>
                    <button class="print-btn" onclick="window.print()"><i class="fas fa-print"></i> Print Report</button>
                    <a href="admin_sports.php" style="background: #6c757d; color: white; padding: 0.5rem 1rem; border-radius: 5px; text-decoration: none;"><i class="fas fa-arrow-left"></i> Back</a>
                </div>
            </div>
            ...
            <div class="filter-bar no-print">
                <form method="GET" style="display: flex; gap: 1.5rem; flex-wrap: wrap; width: 100%; align-items: flex-end;">
                    ...
                    <div class="filter-group">
                        <label><i class="fas fa-filter"></i> Sport/Event Category</label>
                        <select name="category">
                            <option value="all" <?php echo $selected_category == 'all' ? 'selected' : ''; ?>>All Sports/Athletic</option>
                            <?php foreach($availableCategories as $cat): ?>
                                <option value="<?php echo $cat['id']; ?>" <?php echo $selected_category == $cat['id'] ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($cat['category_name']); ?> (<?php echo $cat['category_type']; ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="filter-group">
                        ...
                        <a href="admin_sports_report.php"><i class="fas fa-eraser"></i> Clear Filters</a>
                    </div>
                </form>
            </div>
            ...
            <div class="report-container" id="reportContainer">
                ...
                <div class="report-content">
                    ...
                    <div class="report-header">
                        <div style="display: flex; justify-content: space-between; align-items: center; gap: 2rem; flex-wrap: wrap;">
                            ...
                            <div style="display: flex; align-items: center; gap: 1.5rem;">
                                ...
                                <p>Republic of the Philippines<br><strong>BOHOL ISLAND STATE UNIVERSITY</strong><br>Sport and Development Office<br>Cogtong, Candijay, 6312 Bohol, Philippines<br>Balance | Integrity | Stewardship | Uprightness</p>
                            </div>
                    ...
                    <div class="report-meta">
                        ...
                        <div><strong><i class="fas fa-id-card"></i> Admin Type:</strong> Sports Admin</div>
                        <div><strong><i class="fas fa-filter"></i> Report Type:</strong> Sports & Athletic Only</div>
                    ...
                    <div class="report-meta" style="background: #e9ecef; border-bottom: 1px solid #dee2e6;">
                        ...
                        <div class="filter-info"><i class="fas fa-filter"></i> Sport/Event: <strong><?php echo htmlspecialchars($selected_category_name); ?></strong></div>
                    ...
                    <div class="stats-summary">
                        <div class="stat-box"><div class="number"><?php echo $stats['total_coaches']; ?></div><div class="label"><i class="fas fa-chalkboard-user"></i> Sports Coaches</div></div>
                        <div class="stat-box"><div class="number"><?php echo $stats['total_teams']; ?></div><div class="label"><i class="fas fa-users"></i> Sports Teams</div></div>
                        <div class="stat-box"><div class="number"><?php echo $stats['total_participants']; ?></div><div class="label"><i class="fas fa-user-graduate"></i> Athletes</div></div>
                        <div class="stat-box"><div class="number"><?php echo $stats['participants_with_rankings']; ?></div><div class="label"><i class="fas fa-medal"></i> With Rankings</div></div>
                    </div>
                    
                    <!-- COACHES SECTION -->
                    <div class="report-section">
                        <h2 class="section-title"><i class="fas fa-chalkboard-user"></i> Sports Coaches</h2>
                            ...
                            <?php foreach($coaches as $coach): ?>
                                <div class="coach-card">
                                    ...
                                    <div class="coach-details">
                                        ...
                                        <i class="fas fa-user-graduate"></i> Athletes: <?php echo $coach['total_members']; ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p style="color: #999; text-align: center; padding: 1rem;">No sports coaches found with the selected filters.</p>
                        <?php endif; ?>
                    </div>
                    
                    <!-- TEAMS SECTION -->
                    <div class="report-section">
                        <h2 class="section-title"><i class="fas fa-users"></i> Sports Teams by Activity/Event</h2>
                        <?php if(count($teams_by_activity) > 0): ?>
                                ...
                                <table class="data-table">
                                    <thead>
                                        <tr>
                                            <th>Team Name</th>
                                            <th>Coach</th>
                                            <th>Contest/Tournament</th>
                                            <th>Category</th>
                                            <th>College Filter</th>
                                    ...
                                    <tbody>
                                        <?php foreach($activity['teams'] as $team): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if($team['category_type'] == 'Sport'): ?>
                                                        <span class="category-badge-sport"><?php echo htmlspecialchars($team['category_name']); ?></span>
                                                    <?php else: ?>
                                                        <span class="category-badge-athletic"><?php echo htmlspecialchars($team['category_name']); ?></span>
                                                    <?php endif; ?>
                                                </td>
                                                <td>
                                                    <?php 
                                                    if(!empty($team['college_filter'])) {
                                                        echo '<strong>' . htmlspecialchars($team['college_filter']) . '</strong>';
                                                    } else {
                                                        echo '<span style="color: #999;">All Colleges</span>';
                                                    }
                                                    ?>
                                                </td>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p style="color: #999; text-align: center; padding: 1rem;">No sports teams found with the selected filters.</p>
                        <?php endif; ?>
                    </div>
                    
                    <!-- PARTICIPANTS SECTION -->
                    <div class="report-section">
                        <h2 class="section-title"><i class="fas fa-user-graduate"></i> Athletes and Participants</h2>
                        <?php if(count($participants_by_activity) > 0): ?>
                                    ...
                                    <tbody>
                                        <?php foreach($activity['participants'] as $participant): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if($participant['category_type'] == 'Sport'): ?>
                                                        <span class="category-badge-sport"><?php echo htmlspecialchars($participant['category_name']); ?></span>
                                                    <?php else: ?>
                                                        <span class="category-badge-athletic"><?php echo htmlspecialchars($participant['category_name']); ?></span>
                                                    <?php endif; ?>
                                                </td>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p style="color: #999; text-align: center; padding: 1rem;">No athletes found with the selected filters.</p>
                        <?php endif; ?>
                    </div>
                    
                    <!-- Footer -->
                    <div class="footer">
                        <p><i class="fas fa-print"></i> Generated by BISU Candijay Achievement Tracker System | Sports Development Division</p>
